...
})({"01n0j":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "8eaee4cde0c18625";
...
},{}],"1oOKd":[function(require,module,exports) {
...
function pleaseCapitalizeFirstLetter(name) {
    return name.charAt(0).toUpperCase() + name.slice(1);
}
function changeTitle(pokemon) {
    const title = document.querySelector("title");
    if (title != null) title.textContent = pleaseCapitalizeFirstLetter(pokemon.name);
    else throw new Error("Container is not currently defined");
}
function addPokemonFrontAndBackImage(pokemon) {
    const pokePics = document.querySelector("figure");
    if (pokePics != null) pokePics.innerHTML = `
...
function displayPokemonName(pokemon) {
    if (pokeName != null) pokeName.textContent = pleaseCapitalizeFirstLetter(pokemon.name);
    else throw new Error("Container is not currently defined");
}
function findEnglishLanguage(arrayOfObjects) {
    const englishObject = arrayOfObjects.find((object)=>object.language.name === "en"
    );
    if (englishObject != null) return englishObject.short_effect;
    else throw new Error("Container is not currently defined");
}
function createListItem(abilityDetailObject) {
    ...
    if (pokeAbilityList != null) pokeAbilityList.append(li);
    else throw new Error("Container is not currently defined");
}
function changeBackgroundColor(pokemon) {
    const typeBackground = document.querySelector(".type-background");
    if (typeBackground != null) typeBackground.classList.add(`${pokemon.types[0].type.name}`);
    else throw new Error("Could not find background");
}
fetch(`https://pokeapi.co/api/v2/pokemon/${queryString.get("pokemon")}`).then((response)=>response.json()
    ...
    const abilities = parsedResponse.abilities.map((abilityObject)=>abilityObject.ability
    );
    ...
    return Promise.all(fetches).then((responses)=>{
        ...
        else throw new Error("Spinner is not currently defined");
    });
});
exports.default = {};
...
},{}]},["01n0j","1oOKd"], "1oOKd", "parcelRequire80db")

//# sourceMappingURL=pokemon.e0c18625.js.map
